...
파일을 무조건 Read로 여는 것도 낭비다. `~/.claude`에 문서가 쌓이면서 새로운 문제가 생겼다. 필요한 정보가 어느 파일에 있는지 알기 위해 먼저 파일을 열어봐야 하는 상황이 반복됐다.

QMD 플러그인이 이 문제를 해결한다. BM25 키워드 검색과 벡터 유사도 검색을 조합해 로컬 마크다운 문서를 색인한다. Read로 파일을 열기 전에 반드시 `qmd:qmd` 스킬로 먼저 검색한다. 검색 결과에 충분한 정보가 있으면 파일을 열지 않는다.
...
키워드 검색(`lex`)은 정확한 용어를 알 때, 벡터 검색(`vec`)은 의미 기반으로 찾을 때 사용한다. 두 방식을 조합하면 단독 검색보다 정밀도가 높아진다.
